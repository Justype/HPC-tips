...
### Create a overlay and install Miniforge3

```bash
# create a 20 GiB overlay image
singularity overlay create -s 20000 --sparse $SCRATCH/container/no-ood.img
...
# >>> conda initialize >>>
# !! Contents within this block are managed by 'conda init' !!
__conda_setup="$('/ext3/miniforge3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/ext3/miniforge3/etc/profile.d/conda.sh" ]; then
...
if [ -f "/ext3/miniforge3/etc/profile.d/mamba.sh" ]; then
    . "/ext3/miniforge3/etc/profile.d/mamba.sh"
fi
# <<< conda initialize <<<
```
...
singularity exec\
  ...
  /usr/lib/rstudio-server/bin/rserver \
    ...
    --rsession-which-r=/ext3/miniforge3/bin/R \
    --rsession-ld-library-path=/ext3/miniforge3/lib \
    --www-address=127.0.0.1 \
    --www-port=8080 \
    --server-daemonize=0 \
    --auth-none=1
